...
很多人打开 CMD 直接就以**绝对路径**运行 CFST 会报错，这是因为显式使用本地 IP 段文件（例如 `-f ip.txt` 或 `-cfip=false` 回退本地缓存）时，`ip.txt` 是相对路径，需要指定绝对路径的 ip.txt 才行。为了简单起见，仍然建议进入 CFST 程序目录下，以**相对路径**方式运行：
...
# 注意！如果显式使用本地 IP 段文件，快捷方式 - 起始位置 不能是空的，否则可能因为绝对路径而找不到 ip.txt 文件
```
...
****
``` bash
# 默认从 Cloudflare 官网实时获取 IPv4 IP 段测速，并更新 ip.txt 缓存
cfst

# 强制使用本地 IPv4 缓存文件测速
cfst -f ip.txt

# 或关闭 Cloudflare 官网实时获取，回退到本地 ip.txt 缓存
cfst -cfip=false

# 指定本地 IPv6 缓存文件可测速这些 IPv6 地址
# 另外，v2.1.0 版本后支持 IPv4+IPv6 混合测速并移除了 -ipv6 参数，因此一个文件内可以同时包含 IPv4+IPv6 地址
cfst -f ipv6.txt

# 默认在线获取时，同时包含 Cloudflare IPv6 IP 段，并更新 ip.txt / ipv6.txt 缓存
cfst -cfipv6
